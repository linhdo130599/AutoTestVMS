...
//    driver = new ChromeDriver();
  ...
  public void normal1() throws InterruptedException {
    ...
    driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(3000));
    driver.findElement(By.name("Passwd")).sendKeys("vnn123456");
...
//    driver.close();
    ...
    {
        String time = new SimpleDateFormat("yyyy/MM/dd_HH:mm:ss").format(Calendar.getInstance().getTime());
        System.out.println("Click otp input... ~ " + time);
    }
    driver.findElement(By.name("otp")).click();

    //Sử dụng javascript executor để mở một tab mới và truy cập website gmail
    js.executeScript("window.open('https://mail.google.com/mail/u/0/?tab=rm&ogbl#inbox','_blank');");
    //Lấy các tabs hiện tại đang có và switch qua tab có index = 1, lúc này gmail đang ở tab thứ 2 nên sẽ có index = 1
    ArrayList<String> tabs = new ArrayList<String>(driver.getWindowHandles());
    //Switch qua tab gmail vừa mở
...
//    {
//      WebElement element = driver.findElement(By.xpath("//div[@aria-label='Làm mới']//div[@class='asa']"));
//      Actions builder = new Actions(driver);
//      builder.moveToElement(element).perform();
//    }
    //Click vào mail để mở nội dung email
    TimeUnit.SECONDS.sleep(10);
    String otpSubject = "VietNamNet OTP";
    WebElement otpEmail = driver.findElement(By.xpath("//span[contains(text(), '" + otpSubject + "')]/ancestor::tr"));
    otpEmail.click();
    // Trích xuất mã OTP từ nội dung email bằng regex
    WebElement emailContent = driver.findElement(By.xpath("//div[@class='a3s aiL ']"));
    String otpRegex = "\\b\\d{6}\\b"; // Mã OTP có 6 chữ số
    Pattern pattern = Pattern.compile(otpRegex);
    Matcher matcher = pattern.matcher(emailContent.getText());
    {
      if (matcher.find()){
      String time = new SimpleDateFormat("yyyy/MM/dd_HH:mm:ss").format(Calendar.getInstance().getTime());
      System.out.println("OTP:  ~ " + matcher.group(0)+ " ~ Time: " + time);
      }
    }
...
//    confirmationCode =emailContent.substring(matcher.start(), matcher.end());
    ...
    //Switch về lại tab VMS (index = 0)
    driver.switchTo().window(tabs.get(0));
    //Điền confirmation code
    if (matcher.find()) {
      driver.findElement(By.xpath("//input[@name='code']")).sendKeys(matcher.group(0));
    }
    //chờ 15s để nhập
    TimeUnit.SECONDS.sleep(10);
    {
        String time = new SimpleDateFormat("yyyy/MM/dd_HH:mm:ss").format(Calendar.getInstance().getTime());
        System.out.println("Finished 15s! ~ " + time);
    }
    //OTP number
...
//    TimeUnit.SECONDS.sleep(5);
    ...
    wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector(".svg-icon-danger > svg")));
    System.out.println("Waiting for loading menu...");
    try {
        WebElement element = driver.findElement(By.cssSelector(".svg-icon-danger > svg"));
        element.click();
    }
    ...
    // chỗ này trở lên đang bị lỗi khi mà đã thay button của click menu, thay đổi thực hiện hover-> click cố định menu
...
//    TimeUnit.SECONDS.sleep(3);
    ...
    wait.until(ExpectedConditions.elementToBeClickable(By.id("kt_content")));
...
//    new WebDriverWait(driver, Duration.ofSeconds(5)).until(ExpectedConditions.elementToBeClickable(By.cssSelector("button[title='Chọn ảnh'][aria-label='Chọn ảnh']")));
    ...
    {
        String time = new SimpleDateFormat("yyyy/MM/dd_HH:mm:ss").format(Calendar.getInstance().getTime());
        System.out.println("Sắp Click can giua... ~ " + time);
    }
//    TimeUnit.SECONDS.sleep(5);
    wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("button[title='Canh giữa'][aria-label='Canh giữa']")));
...
//  js.executeScript("window.scrollTo(0,0)");
    ...
    {
        String time = new SimpleDateFormat("yyyy/MM/dd_HH:mm:ss").format(Calendar.getInstance().getTime());
        System.out.println("Đang chọn Category... ~ " + time);
    }
    wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//span[contains(text(),'Chọn mục chính')]")));
...
//    wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector(".select2-search--dropdown > .select2-search__field")));
...
//    TimeUnit.SECONDS.sleep(5);
    wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector(".btn-elevate:nth-child(2)")));
